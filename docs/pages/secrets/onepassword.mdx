...
Research tools use the same item-title and `credential` field contract. Enable
only the providers your deployment intends to use:

| Credential | Used for |
|------------|----------|
| `EXA_API_KEY` | Raw `websearch search` retrieval through Exa. |
| `OPENROUTER_API_KEY` | `websearch` synthesis through OpenRouter-compatible chat completions. |
| `FIRECRAWL_API_KEY` | `firecrawl search` and `firecrawl scrape`. |
| `ANTHROPIC_API_KEY` | Optional Anthropic compatibility for `websearch` synthesis. |
| `SUPERMEMORY_API_KEY` | Supermemory-backed memory tools or MCP bridges when enabled. |

Non-secret model selection, such as `OPENROUTER_MODEL`,
`WEBSEARCH_SYNTHESIS_PROVIDER`, or `DEEP_RESEARCH_MODEL`, belongs in ordinary
deployment config rather than 1Password unless your environment treats model
names as sensitive.
