...
This repo also includes `overlays/raava-internal` as a local dogfood overlay.
Use it when you want a concrete example with personas, a tool, workflow
shadowing, and manager delegation in one place before forking the ACME template.
Treat that Raava directory as an overlay scaffold only: do not push
Raava-specific branches to upstream `paradigmxyz/centaur`; publish them through
a Raava-owned overlay repo or fork.
