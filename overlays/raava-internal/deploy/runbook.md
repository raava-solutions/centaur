...
> **Written against the pre-Rust platform.** Command sequences here reference the
> deleted Python `services/api`, the deleted `services/slackbot`, and chart keys
> (`api:`, `slackbot:`, `tokenBroker:`, `overlay.image`) that the current chart
> ignores silently. The infrastructure history and the incident notes are still
> accurate; the apply commands are not. Follow `RELEASE.md` in this directory for
> anything you intend to run, and treat each command here as needing translation
> to `apiRs:` / `slackbotv2:` / `toolServer.extraSources`. The working tree this
> runbook described is preserved as the `archive/hermes-slack-20260821` tag.
