...
def test_raava_personas_are_discoverable_without_private_specialists() -> None:
    ...
    assert "raava_rbe" in manager.tools
    assert "proposal-first" in manager.get_persona("argus").prompt_content
    assert (
        "do not report to the engineering persona"
        in manager.get_persona("vivian").prompt_content.lower()
    )


def test_raava_rbe_tool_uses_local_baseline(monkeypatch) -> None:
    monkeypatch.setenv("RAAVA_RBE_BASE_URL", "")
    client_module = _load_module(
        "test_raava_rbe_client",
        OVERLAY_ROOT / "tools" / "raava_rbe" / "client.py",
    )
    client = client_module.RaavaRbeClient()
...
def test_raava_rbe_tool_calls_cloudflare_mcp_with_static_token(monkeypatch) -> None:
    monkeypatch.setenv("RAAVA_RBE_BASE_URL", "https://brain.raava.dev")
    client_module = _load_module(
        "test_raava_rbe_client_mcp",
        OVERLAY_ROOT / "tools" / "raava_rbe" / "client.py",
    )
    client = client_module.RaavaRbeClient()
    client.api_key = "rbe-read-token"
    ...
    def fake_post(url, *, json, headers, timeout):
        ...
        payload = {
            "jsonrpc": "2.0",
            "result": {
                "query": "FitRoofingCo recent call notes",
                "results": [
                    {
                        "slug": "companies/fit-roofing",
                        "title": "FitRoofingCo",
                    }
                ],
            },
    ...
    assert calls[0]["url"] == "https://brain.raava.dev/mcp"
    assert calls[0]["json"] == {
        "jsonrpc": "2.0",
        "id": 1,
        "method": "tools/call",
        "params": {
            "name": "query",
            "arguments": {"q": "FitRoofingCo recent call notes", "limit": 3},
        },
    }
    assert calls[0]["headers"]["authorization"] == "Bearer rbe-read-token"
    assert result["source"] == "rbe"
    assert result["results"][0]["slug"] == "companies/fit-roofing"
